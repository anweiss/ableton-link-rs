...
* **Memory Safe**: Leverages Rust's ownership system for safe concurrent networking. The package sets `unsafe_code = "deny"` in `[lints.rust]`, covering examples and tests as well as the library. Platform integration uses wrapper crates where available; narrowly scoped exceptions cover Linux/Windows discovery egress socket options and Windows example console mode, with alternatives and safety contracts documented at each exception
* **Ingress-Aware Discovery**: On Linux, macOS and Windows, managed discovery receives the OS interface index with each datagram and selects that interface's registered response socket, not a source-address prefix match. Responses are pinned to that adapter; observed removal/replacement invalidates pending sends. See [discovery interface behavior and limitations](docs/discovery-ingress.md), including polling blind spots, duplicate local addresses and unexercised multihomed platform behavior
