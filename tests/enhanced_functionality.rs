...
use ableton_link_rs::link::{
    clock::Clock, host_time_filter::HostTimeFilter, linear_regression::linear_regression,
    median::median_from_iter, BasicLink,
};
...
fn test_clock_synchronization_components() {
    ...
    assert!(
        (slope - 2.0).abs() < 1e-10,
        "Linear regression slope should be 2.0"
    );
    assert!(
        (intercept - 1.0).abs() < 1e-10,
        "Linear regression intercept should be 1.0"
    );
    ...
    assert!(
        median_result > 99.0 && median_result < 105.0,
        "Median should filter out outlier"
    );
    ...
    assert!(
        filter.is_ready(),
        "Host time filter should be ready after enough samples"
    );
    assert_eq!(
        filter.num_points(),
        10,
        "Should be limited by filter capacity"
    );
...
async fn test_enhanced_basic_link() {
    ...
    assert!(
        *callback_invoked.lock().unwrap(),
        "Tempo callback should be invoked on setup"
    );
    assert!(
        *peer_callback_invoked.lock().unwrap(),
        "Peer count callback should be invoked on setup"
    );
    assert!(
        *start_stop_callback_invoked.lock().unwrap(),
        "Start/stop callback should be invoked on setup"
    );
    ...
    assert!(
        ...
        "Should initially not be playing"
    );
...
async fn test_realtime_performance() {
    ...
    assert!(
        ...
        "Should handle high-frequency real-time operations"
    );
